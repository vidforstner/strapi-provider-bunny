...
const init = ({
  ...
  const upload = async file => {
    ...
    try {
      const response = await _axios.default.put(`https://${hostname}/${storage_zone}/${file.hash}${file.ext}`, data, {
        headers: {
          AccessKey: api_key,
  ...
  const download = async file => {
    try {
      const response = await _axios.default.get(`https://${hostname}/${storage_zone}/${file.hash}${file.ext}`, {
        headers: {
          AccessKey: api_key
        },
  ...
  const deleteFile = async file => {
    try {
      const response = await _axios.default.delete(`https://${hostname}/${storage_zone}/${file.hash}${file.ext}`, {
        headers: {
          AccessKey: api_key
        }
      });
