...
const init = ({
  ...
  const upload = async file => {
    ...
    try {
      const response = await _axios.default.put(`${hostname}/${storage_zone}/${file.hash}${file.ext}`, data, {
        headers: {
          AccessKey: api_key,
          // Checksum: checksum,
  ...
  const download = async file => {
    try {
      const response = await _axios.default.get(`${hostname}/${storage_zone}/${file.hash}${file.ext}`, {
        headers: {
          AccessKey: api_key
        },
  ...
  const deleteFile = async file => {
    try {
      const response = await _axios.default.delete(`${hostname}/${storage_zone}/${file.hash}${file.ext}`, {
        headers: {
          AccessKey: api_key
        }
      });
